...
def AB_analysis(z,t):
    ...
    #CB0 = C/B0
    #HB0 = ABB0+CB0
    #if z.imag==0: return (H.real,H.real/B0,abs(H.real/B0),ABB0,abs(ABB0),CB0,abs(CB0)) 
    #else: return (H,HB0,abs(HB0),ABB0,abs(ABB0),CB0,abs(CB0))
...
def adjusted_AB_analysis(z,t):
    ...
    A_sum = 0.0
    for n in range(1,N+1):
        if t.real>0: A_sum += mp.exp((t/16)*mp.power(mp.log(s_like1/(mp.pi()*n*n)),2))/mp.power(n,s)
        else: A_sum += 1/mp.power(n,s) 
    A = A_pre*A_sum
    ...
    ddxB_sum = 0.0
    for m in range(1,M+1):
        if t.real>0: mth_term = mp.exp((t/16)*mp.power(mp.log(s_like2/(mp.pi()*m*m)),2))/mp.power(m,1-s) 
        else: mth_term = 1/mp.power(m,1-s)
        B_sum += mth_term
