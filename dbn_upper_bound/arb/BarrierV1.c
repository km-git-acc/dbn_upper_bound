...
{
    arb_t X, y0, t0, dxabb, dtabb, a, t, modabb, minmodabb, windnum, windtot;
    acb_t cX, cy0, ct0, ct, argdiv;
    acb_mat_t res;

    const char *X_str, *y0_str, *t0_str, *expterms_str, *prt_str;
    slong N_low, N, N_high, prec, H, expterms, rectmesh, idx, count, prt;
...
//    prt_str = "1";
    ...
    N_low = get_N(t0, X-1, prec);
    N_high = get_N(t0, X-2, prec);
    if (N_low != N_high)
    {
        fprintf(stderr, "Your choice of X lies exactly on a jump\n");
        fprintf(stderr, "from N to N+1. Please select another X.\n");
        result = EXIT_FAILURE;
        goto finish;
    }
