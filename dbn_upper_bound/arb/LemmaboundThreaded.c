...
arb_poly_t tpoly;
arb_poly_t modgam;
arb_poly_t modgamend;
arb_poly_t sig;
arb_poly_t sumnd;

struct ThreadData {
slong start, stop, id, prec, divs, Nmin, n;
slong* d;
}; 
...
static void
_arb_poly_eBs_series(arb_poly_t eout, const arb_poly_t xN, const arb_poly_t t, const arb_poly_t y, slong Nb, slong n, slong prec)
{
    arb_t Nbarb, logNb, pi;
    arb_init(logNb); 
    arb_init(Nbarb); 
    arb_init(pi);
    arb_const_pi(pi, prec); 

    arb_poly_t a, b, c, pip, summand1, summandNb, Nbpoly, zerod626, sixd66;
    arb_poly_init(a);
    arb_poly_init(b);
    arb_poly_init(c);
    arb_poly_init(summand1);
    arb_poly_init(summandNb);
    arb_poly_init(Nbpoly);
    arb_poly_init(pip);
    arb_poly_init(zerod626);
    arb_poly_init(sixd66);

    arb_poly_set_arb(pip, pi);   

    arb_poly_set_si(a, 626);
    arb_poly_set_si(b, 1000);
    arb_poly_div_series(zerod626, a, b, 1, prec);

    arb_poly_set_si(a, 666);
    arb_poly_set_si(b, 100);
    arb_poly_div_series(sixd66, a, b, 1, prec);

    arb_set_si(Nbarb, Nb);
    arb_log(logNb, Nbarb, prec);
    arb_poly_set_arb(Nbpoly, Nbarb);

    arb_poly_pow_ui(b, t, 2, prec);
    arb_poly_scalar_mul_2exp_si(b, b, -4);
  
    arb_poly_scalar_mul_2exp_si(c, pip, 2);  
    arb_poly_div_series(c, xN, c, 1, prec);    
    arb_poly_log_series(c, c, 1, prec); 
    arb_poly_pow_ui(c, c, 2, prec);  

    arb_poly_mullow(c, c, b, 1, prec);
    arb_poly_add_series(c, c, zerod626, 1, prec);  

    arb_poly_sub_series(b, xN, sixd66, 1, prec);  

    arb_poly_div_series(c, c, b, 1, prec); 
    arb_poly_exp_series(c, c, 1, prec); 
    arb_poly_add_si(summand1, c, -1, prec);

    _arb_poly_bt_series(a, logNb, t, 1, prec);
    _arb_poly_sig_series(b, xN, t, y, 1, prec);
    arb_poly_neg(b,b);
   
    arb_poly_pow_series(c, Nbpoly, b, 1, prec);
    arb_poly_mullow(a, c, a, 1, prec);

    arb_poly_pow_ui(b, t, 2, prec);
    arb_poly_scalar_mul_2exp_si(b, b, -4);

    arb_poly_pow_ui(c, Nbpoly, 2, prec);  
    arb_poly_scalar_mul_2exp_si(c, c, 2);
    arb_poly_mullow(c, c, pip, 1, prec);   
    arb_poly_div_series(c, xN, c, 1, prec);    
    arb_poly_log_series(c, c, 1, prec); 
    arb_poly_pow_ui(c, c, 2, prec);  
    ...
    arb_poly_mullow(summandNb, c, a, 1, prec);

    arb_poly_max_series(eout, summand1, summandNb, 1, prec);

    arb_poly_set_si(a, 3);
    arb_poly_mullow(eout, eout, a, 1 , prec);
    arb_poly_mullow(eout, eout, Nbpoly, 1 , prec);
    ...
    arb_poly_clear(b);
    arb_poly_clear(c);
    arb_poly_clear(summand1);
    arb_poly_clear(summandNb);
    arb_poly_clear(Nbpoly);
    arb_poly_clear(pip);
    arb_poly_clear(zerod626);
...
//procedure to base bound sums for an individual thread
void* abbeff_base_bound_loop(void *voidData)
{
    ...
    slong divs=data->divs;
    slong Nmin=data->Nmin;
    slong n=data->n;
    slong* d=data->d;

    slong j, k, nd;

    arb_poly_t atpoly, btpoly, ndsum_a, ndsummand_a, ndsum_b, ndsummand_b, sumnd, ndsummand, summandnd, kterm;
    arb_poly_init(btpoly);
    arb_poly_init(atpoly);
    arb_poly_init(ndsum_a);
    arb_poly_init(ndsummand_a);
    ...
    //main loop from n=1 to D*N -1
    arb_poly_zero(sumnd);
    for (k = start; k < stop; k++)
    {
        arb_poly_zero(ndsum_a);
        arb_poly_zero(ndsum_b);
        //sub-loop from nd=1 to divs
        for (nd = 1; nd <= divs; nd++)
        {
            if (((k+1) % d[nd] == 0) && ((k+1) <= d[nd]*Nmin))
            {
                ...
                _arb_poly_bt_series(btpoly, logj, tpoly, n, prec);
                arb_poly_mullow(ndsummand_b, moll + nd, btpoly, n, prec);
                arb_poly_add_series(ndsum_b, ndsum_b, ndsummand_b, n, prec);

                _arb_poly_at_series(atpoly, logj, tpoly, ypoly, n, prec);
                arb_poly_mullow(ndsummand_a, moll + nd, atpoly, n, prec);
                arb_poly_mullow(ndsummand_a, ndsummand_a, modgam, n, prec);
                arb_poly_add_series(ndsum_a, ndsum_a, ndsummand_a, n, prec);
            }
    ...
    //store the ndsum per thread
    arb_one(ab);
    arb_poly_evaluate(ab, sumnd, ab, prec); 
    arb_set(arb_mat_entry(threadsum, id, 0), ab);
...
void
arb_poly_abbeff_base_lbound(arb_poly_t lbound, const arb_poly_t t, const arb_poly_t y, arb_poly_srcptr moll,
                            slong nprimes, const ulong *primes, slong *d, slong D, slong Nmin, 
                            arb_poly_t modgamend, slong numthreads, slong n, slong prec)
{
    slong divs;
    arb_poly_t a, b;
    arb_poly_t modK, ndsummand, sum2;
    arb_poly_t Nminpoly, Nmaxpoly;
    arb_poly_t xN, kterm, kterm1, ndterm, modmol, btpoly;
    ...
    slong threadtasks = (D*Nmin+numthreads-1)/numthreads;
    for (i = 0; i < numthreads; i++)
    {
       data[i].start=i*threadtasks+1;
    ...
    data[numthreads-1].stop = D*Nmin;

    arb_mat_init(threadsum, numthreads, 1);

    //start the threads with an indexed (array) of a data-structure (with pointers to it from the threads).
    for (i = 0; i < numthreads; i++)
    {
       pthread_create(&thread[i], 0, abbeff_base_bound_loop, &data[i]);
    }
    ...
    //sum all sumnd's per thread together
    arb_poly_zero(sumnd);
    for (i = 0; i < numthreads; i++)
    {
       arb_poly_set_arb(a, arb_mat_entry(threadsum, i, 0));
       arb_poly_add_series(sumnd, sumnd, a, n, prec);
    }
...
main_loop(slong res, const arb_t t, const arb_t y, slong Na, slong Nb, slong Nprint,
          slong nprimes, arb_t threshold, slong mode, slong numthreads, slong digits, slong n, slong prec)
{
    ...
    arb_poly_t btpoly, xN, xN0, eA, eB, eC0, eTot, lboundfull, lboundinc, Napoly, Nbpoly, nextNpoly, prevbound;
    arb_poly_init(tpoly);
    arb_poly_init(ypoly);
    arb_poly_init(btpoly);
    arb_poly_init(xN0);
    ...
    //establish lower error bounds first time
    _arb_poly_xNpoly_series(xN, Napoly, tpoly, n, prec);
    _arb_poly_eC0_series(eC0, xN, tpoly, ypoly, Na, n, prec);
    _arb_poly_eBs_series(eB, xN, tpoly, ypoly, Nb, n, prec);

    arb_poly_add_series(eTot, eB, eC0, 1, prec);

    //main loop from N-start to N-end 
    nextN = Na;
    _arb_poly_modgamend_series(modgamend, Nbpoly, tpoly, ypoly, n, prec);

    arb_poly_abbeff_base_lbound(lboundfull, tpoly, ypoly, moll, nprimes, primes, d, D, Na, modgamend, numthreads, 1, prec);
    ...
    while (nextN < Nb)
    {
        ...
        arb_poly_evaluate(a, lboundinc, a, prec);
         ...
         //check recalcuation need against threshold
        if (arb_lt(a, threshold))
        {
            arb_poly_abbeff_base_lbound(lboundfull, tpoly, ypoly, moll, nprimes, primes, d, D, nextN, modgamend, numthreads, 1, prec);
            arb_poly_set(prevbound, lboundfull);
            arb_poly_set_si(nextNpoly, nextN);
            flint_printf("%ld, ", nextN);
            arb_poly_get_coeff_arb(coeff, lboundfull, 0);
            arf_printd(arb_midref((coeff)), digits);
            flint_printf(", ");  
            _arb_poly_xNpoly_series(xN, nextNpoly, tpoly, n, prec);
            _arb_poly_eC0_series(eC0, xN, tpoly, ypoly, nextN, n, prec);
            arb_poly_add_series(eTot, eB, eC0, 1, prec);
            arb_poly_get_coeff_arb(coeff, eTot, 0);
            arf_printd(arb_midref(coeff), digits);
...
int main(int argc, char *argv[])
{
    ...
    const char *m_str, *thres_str, *mode_str, *thread_str, *d_str;

    slong Na, Nb, Nprint;
    slong m, mode, d;
    slong prec, res, numthreads;
    ...
    if (Na < 1 || Nb < 1 || Nprint < 1 || Na > Nb || numthreads < 1 ||
        m < 0 || m > 20 || mode < 0 || mode > 1 ||d < 1)
    {
        usage_error = 1;
        result = EXIT_FAILURE;
        goto finish;
...
TIMEIT_ONCE_START
    ...
    main_loop(res, t, y, Na, Nb, Nprint, m, threshold, mode, numthreads, d, 1, prec);
...
finish:
    ...
    {
        flint_printf("Usage:\n");
        flint_printf("%s t y Na Nb Nprint m threshold mode numthreads d\n\n", argv[0]);
        flint_printf(
    "This threaded script computes a lower Lemma bound of abs(Aeff + Beff)/abs(Beff0)\n"
    "for N between 'Na' and 'Nb'. It uses an Euler mollification that includes\n"
    "m primes, so that for example m=0 corresponds to no mollification and m=3\n"
    "corresponds to an Euler mollification that uses the first 3 primes {2, 3, 5}.\n"
    "When the incremental bounds reach a certain 'threshold' then the fully\n"
    "accurate bound is recalculated (labelled with 'recalc').\n"
    "Each row of output consists of N followed by the bound and a conservative \n"
    "upper error bound. The output is only displayed when 'N mod Nprint == 0'.\n"
    "The script can be run in 2 'modes': .\n"
    "Mode 0 = lower lemma bound and conservative upper error bounds are printed.'\n"
    "Mode 1 = exact eA, eB, eCO and total error bounds are printed at 'Nprint'.\n"
    "With numthreads the number of threads to be used can be set.\n"
    "The number of decimal digits can be set by d.\n");
    }
