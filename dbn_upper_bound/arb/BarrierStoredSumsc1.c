...
storedsums(slong res, const acb_t cX, const acb_t cy, 
           const slong H, const slong taylorterms, const slong expterms, slong digits, slong n, slong prec)

{
    ...
    acb_get_real(ab, cX);
    arb_printn(ab, 30, ARB_STR_NO_RADIUS);
    printf(", ");
    acb_get_real(ab, cy);
    arb_printn(ab, 10, ARB_STR_NO_RADIUS);
    printf("\n");
    ...
    for (e = 0; e < expterms; e++)
    {
         for (t = 0; t < taylorterms; t++)
         {
              ...
              arb_printn(re, digits, ARB_STR_NO_RADIUS);
              printf(", ");
              arb_printn(im, digits, ARB_STR_NO_RADIUS);
              if (t < taylorterms - 1) 
                  printf(", ");
              else
                  printf("\n");
...
int main(int argc, char *argv[])
{
    ...
    const char *X_str, *y0_str, *expterms_str, *taylorterms_str, *digits_str;
    slong N, prec, expterms, taylorterms, res, digits;
...
TIMEIT_ONCE_START
    ...
    storedsums(res, cX, cy0, N, taylorterms, expterms, digits, 1, prec);
...
finish:
    ...
    if (result == EXIT_FAILURE)
    {
        ...
        flint_printf("%s X y0 expterms taylorterms digits \n\n", argv[0]);
        flint_printf(
    "This script computes the stored sums as input '3D-Barrier',\n"
    "calculations. Precision has been set at 100 digits accuracy.\n"
    "It prints the finalmatb and finalmata sums separated by ***.\n"
    "t0 has been fixed at 0. Digits sets the number of output digits.\n");
    }
