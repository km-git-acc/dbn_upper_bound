...
//main procedure to establish the approximate Triangle-bound  
void
arb_poly_abbeff_approx_tbound(arb_poly_t tbound, const arb_poly_t t, const arb_poly_t y,
          ...
          slong nprimes, slong digits, slong n, slong prec)
{
    ...
    if (type == 2)
    {
       arb_poly_abbeff_approx_tbound(bound, tpoly, ypoly, moll, nprimes, primes, d, D, N, N0, 1, prec);
       printf("\n");
       printf("Approximate Triangle bound (with tail sum) ");
       printf("for t=");
       arb_printn(t, 3, ARB_STR_NO_RADIUS);
       printf(", y=");
       arb_printn(y, 3, ARB_STR_NO_RADIUS);
...
int main(int argc, char *argv[])
{
    ...
    if (N < 1 || N0 < 0 || N0 > N || m < 0 || m > 20 || d < 1 || tail < 0 || tail > 1 || type < 0 || type > 2)
    {
        usage_error = 1;
        result = EXIT_FAILURE;
        goto finish;
...
finish:
    ...
    " 2 = Approximate Triangle lower bound for N, N0.\n"
