...
//procedure to fill the ptns vector
static void
fillpntsmat(arb_mat_t pntsmat, const arb_t ts, const arb_t te, slong numpnts, slong prec)
{
    arb_t a, pnts;
    arb_init(a);
    arb_init(pnts);
    ...
    arb_set(arb_mat_entry(pntsmat, 0, 0), ts);

    for (v = 1; v < numpnts; v++)
    {
        arb_sub(a, te, ts, prec);
        arb_mul_2exp_si(a, a, -1*(numpnts-v-1));
        arb_add(a, a, ts, prec);
        arb_set(arb_mat_entry(pntsmat, v, 0), a);
    }
...
//procedure to fill the ddxbound vector
static void
fillddxboundmat(arb_mat_t ddxbmat, const acb_t cX, const acb_t cy, arb_mat_t pntsmat, slong H, slong numpnts, slong n, slong prec)
{
    arb_t ddxbound;
    arb_init(ddxbound);
    acb_t ct;
    ...
    for (v = 0; v < numpnts; v++)
    {
        acb_set_arb(ct, arb_mat_entry(pntsmat, v, 0));
        ddxbound_series(ddxbound, cX, cy, ct, H, n, prec);
        arb_set(arb_mat_entry(ddxbmat, v, 0), ddxbound);
    }
...
//procedure to fill the ddxbound vector
static void
fillddtboundmat(arb_mat_t ddtbmat, const acb_t cX, const acb_t cy, arb_mat_t pntsmat, slong H, slong numpnts, slong n, slong prec)
{
    arb_t ddtbound;
    arb_init(ddtbound);
    acb_t ct;
    ...
    {
        acb_set_arb(ct, arb_mat_entry(pntsmat, v, 0));
        ddtbound_series(ddtbound, cX, cy, ct, H, n, prec);
        arb_set(arb_mat_entry(ddtbmat, v, 0), ddtbound);
    }
...
abbeff_t_loop(slong res, const acb_t cX, const acb_t cy, const arb_t ts, const arb_t te, 
           ...
           const slong linesegs, slong prt, slong n, slong prec)

{
    acb_mat_t ests;
    arb_mat_t ddxbmat, ddtbmat, pntsmat;

    arb_t pi, ab, bb, mrt, mrm, harb, Harb, n0, t, modabb, minmodabb, windnum, windtot, dxabb, dtabb,
          lineddxa, lineddxb, lineddta, lineddtb;
    arb_init(ab);
    arb_init(bb);
    arb_init(mrt);
    arb_init(mrm);
    ...
    slong idx, count, rectmesh, sidemesh, num, numpnts, linecount;
    ...
    //calculating line segments to approximate the ddx and ddtbound curves
    numpnts = linesegs + 1;
    arb_mat_init(pntsmat, numpnts, 1);
    arb_mat_init(ddxbmat, numpnts, 1);
    arb_mat_init(ddtbmat, numpnts, 1);

    if (linesegs == 0)
    {
           printf("The ddx and ddt bounds are recalculated for each t-step...\n");
           printf("\n");
           goto skip;
    }
    else
    {
    printf("Precalculating the ddx and ddt bounds for %ld linesegments...\n", linesegs);
    printf("\n");
    }

    fillpntsmat(pntsmat, ts, te, numpnts, prec);
    fillddxboundmat(ddxbmat, cX, cy, pntsmat, H, numpnts, n, prec); 
    fillddtboundmat(ddtbmat, cX, cy, pntsmat, H, numpnts, n, prec); 

    //perform the t-loop over all the X..X+1, y0..1 rectangles
    arb_set(dxabb, arb_mat_entry(ddxbmat, 0, 0));
    arb_set(dtabb, arb_mat_entry(ddtbmat, 0, 0));
...
skip:
    ...
    while(arb_lt(t, te))
    {
        ...
        arb_get_mid_arb(mrt, t);
        arb_get_mid_arb(mrm, arb_mat_entry(pntsmat, linecount, 0));
        //recalculate the line segments for each t for the ddx/ddt bounds (if needed)
        if (arb_ge(mrt, mrm))
        {

            //establish new line segment for ddxbound
            arb_sub(ab, arb_mat_entry(ddxbmat, linecount+1, 0), arb_mat_entry(ddxbmat, linecount, 0), prec);
            arb_sub(bb, arb_mat_entry(pntsmat, linecount+1, 0), arb_mat_entry(pntsmat, linecount, 0), prec);
            arb_div(lineddxa, ab, bb, prec);
            arb_mul(ab, lineddxa, arb_mat_entry(pntsmat, linecount, 0), prec);
            arb_sub(lineddxb, arb_mat_entry(ddxbmat, linecount, 0), ab, prec);
            //establish new line segment for ddtbound
            arb_sub(ab, arb_mat_entry(ddtbmat, linecount+1, 0), arb_mat_entry(ddtbmat, linecount, 0), prec);
            arb_sub(bb, arb_mat_entry(pntsmat, linecount+1, 0), arb_mat_entry(pntsmat, linecount, 0), prec);
            arb_div(lineddta, ab, bb, prec);
            arb_mul(ab, lineddta, arb_mat_entry(pntsmat, linecount, 0), prec);
            arb_sub(lineddtb, arb_mat_entry(ddtbmat, linecount, 0), ab, prec);
            printf("->");
            linecount = linecount + 1;
        }

        //t-linesegment y=ax+b for ddx and ddt bounds 
        arb_mul(ab, t, lineddxa, prec);
        arb_add(dxabb, ab, lineddxb, prec);
        arb_mul(ab, t, lineddta, prec);
        arb_add(dtabb, ab, lineddtb, prec);
...
skip1:
        ...
        arb_set_si(bb, 1);
        arb_mul_2exp_si(ab, bb, -1);
        arb_sub(ab, minmodabb, ab, prec);
        arb_div(ab, ab, dtabb, prec);
        arb_add(t, t, ab, prec); 
...
//        arb_add(t, t, ab, prec);
    ...
    arb_mat_clear(ddxbmat);
    arb_mat_clear(ddtbmat);
    arb_mat_clear(pntsmat);
...
int main(int argc, char *argv[])
{
    ...
    const char *ts_str, *te_str, *expterms_str, *taylorterms_str, *linesegs_str, *prt_str;
    slong e, t, N, prec, expterms, taylorterms, prt, res, linesize, linesegs;
...
TIMEIT_ONCE_START
    ...
    abbeff_t_loop(res, cX, cy0, ts, te, N, taylorterms, expterms, finalmatb, finalmata, linesegs, prt, 1, prec);
...
finish:
    ...
    if (result == EXIT_FAILURE)
    {
        ...
        flint_printf("%s ts te expterms taylorterms linesegs Prt \n\n", argv[0]);
        flint_printf(
    ...
    "the Exp-terms. Linesegs is the number of linear segments desired to" 
	"approximate the ddx and ddt bounds. If set to 0, the bounds are\n"
	"reclaculated for all t. With Prt the output is controlled:\n"
