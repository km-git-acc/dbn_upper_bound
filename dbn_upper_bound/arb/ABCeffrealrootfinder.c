...
void
acb_Muller_loop(slong res, arb_t xa, arb_t xb, arb_t ta, arb_t tb,
                arb_t tstep, slong sw, slong digits, slong prec)
...
{  
    ...
    while(arb_le(t, tb))
    {
        //find all real roots for this t
        rc = 0;
        arb_zero(xrprev);
        arb_zero(yrprev);
        ...
        while(arb_le(xtest, xb))
        {
            ...
            //if the root has not been found or is complex or the same as the previous or is outside the Muller "window" then search+1
            if (arb_is_zero(xr) || arb_gt(yr, accreq) || (arb_lt(xdiff, accreq) && arb_lt(ydiff, accreq)) || arb_lt(xr, xtest) || arb_gt(xr, xtest4))
            {
                arb_add_si(xtest, xtest, 1, prec);
            }
            //else a new root has been found, so set the xtest a tiny bit (xinc) forward (e.g to find a Lehmer pair)
            else
            {
                
                arb_add(xtest, xr, xinc, prec); 
...
int main(int argc, char *argv[])
{
    ...
    const char *xa_str, *xb_str, *ta_str, *tb_str, *tstep_str, *sw_str, *digits_str;
    slong prec, digits, res, sw;
    ...
    if (sw < 0 || sw > 1 || digits < 1 || arb_le(xb, xa))
    {
        result = EXIT_FAILURE;
        goto finish;
    }
...
//TIMEIT_ONCE_START
    acb_Muller_loop(res, xa, xb, ta, tb, tstep, sw, digits, prec);
//TIMEIT_ONCE_STOP
...
finish:
    ...
    if (result == EXIT_FAILURE)
    {
        ...
        flint_printf("%s xa, xb, ta, tb, tstep, sw, digits \n\n", argv[0]);
        flint_printf(
    "This script attempts to find all real roots of \n"
    "Abc_eff(x+iy)/B0 in the domain xa..xb, ta...tb.\n" 
    "sw = 0 prints the x,t,root, sw = 1 prints t,#roots.\n" 
