...
static void
bexpo_aexpo_afac_bsums_asums(acb_t ests, acb_t sarr, const acb_t ssexpo, const acb_poly_t finpoly, const arb_t t, 
...

{
    ...
    acb_add(ests, a, bsums, prec);

    //establish minimum value of modabb
    acb_abs(ab, ests, prec);
...
void* abbeff_symmetric_rectangle(void *voidData)
{
    ...
    acb_t estsprev1, estsprev2, estsprev3, estsprev4, firstside1, firstside2, firstside3, firstside4, argdiv, ests;
    acb_init(a);
    acb_init(b);
    acb_init(c);
    acb_init(estsprev1);
...
else
{
    ...
    { 
       ...
       if (v==0)
       {
          acb_set(firstside1, ests);
       }
       else
       {
          acb_div(argdiv, estsprev1, ests, prec);
          acb_arg(ab, argdiv, prec);
          arb_add(windnum, windnum, ab, prec);
       }
       acb_set(estsprev1, ests);
       ...
       {      
          ...
          print_details(prtresult, t, ab, ad, ests);
       }
       ...
       if (v==0)
       {
             acb_set(firstside2, ests);
       }
       else
       {
          acb_div(argdiv, estsprev2, ests, prec);
          acb_arg(ab, argdiv, prec);
          arb_add(windnum, windnum, ab, prec);
       }
       acb_set(estsprev2, ests);
       ...
       if (v==0)
       {
          acb_set(firstside3, ests);
       }
       else
       {
          acb_div(argdiv, ests, estsprev3, prec);
          acb_arg(ab, argdiv, prec);
          arb_add(windnum, windnum, ab, prec);
       }
       acb_set(estsprev3, ests);
       ...
       if (v==0)
       {
          acb_set(firstside4, ests);
       }
       else
       {
          acb_div(argdiv, ests, estsprev4, prec);
          acb_arg(ab, argdiv, prec);
          arb_add(windnum, windnum, ab, prec);
       }
       acb_set(estsprev4, ests);
    ...
    //complete the 4 'missing links' for the winding number
    acb_div(argdiv, firstside3, estsprev4, prec);
    acb_arg(ab, argdiv, prec);
    arb_add(windnum, windnum, ab, prec);

    acb_div(argdiv, firstside4, firstside1, prec);
    acb_arg(ab, argdiv, prec);
    arb_add(windnum, windnum, ab, prec);

    acb_div(argdiv, estsprev2, estsprev3, prec);
    acb_arg(ab, argdiv, prec);
    arb_add(windnum, windnum, ab, prec);

    acb_div(argdiv, estsprev1, firstside2, prec);
    acb_arg(ab, argdiv, prec);
    arb_add(windnum, windnum, ab, prec);
    ...
    acb_clear(estsprev3);
    acb_clear(estsprev4);
    acb_clear(firstside1);
    acb_clear(firstside2);
    acb_clear(firstside3);
    acb_clear(firstside4);
    acb_clear(argdiv);
    acb_clear(sarr);
    acb_clear(ests);
    acb_clear(one);
...

{
    ...
    arb_t windtot, dzabb, dtabb, afac, const1;
    arb_init(a);
    arb_init(b);
    arb_init(c);
    arb_init(d);
    ...
    slong i, count, rectmesh, num, prtresult;
    prtresult = 0;
    ...
    while(arb_le(t, te))
    {
        ...
        printf(", %05.10f, ", arf_get_d(arb_midref(windnum), ARF_RND_NEAR));
...
finish: 
    ...
    " 0 = prints rectangle summary only, 1 = prints full details (BUT NOT IN SEQUENCE!).\n"
    " \n");
    }
