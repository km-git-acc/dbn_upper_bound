...
int main(int argc, char *argv[])
{
    ...
    for (tht=0; tht <= numx; tht++)
    {
        ...
        arb_set_si(a, thres);
        if (arb_gt(arb_mat_entry(output, 3, 0), a) && arb_gt(arb_mat_entry(output, 4, 0), a)
		    && arb_gt(arb_mat_entry(output, 5, 0), a))
        { 
            ...
            if (sw == 1)
            {
                for (i=0; i<3; i++)
                {
                   ...
                   arb_add(b, X, arb_mat_entry(offset, i, 0), prec);
                   printf("-> ");
                   arb_printn(b, 10, ARB_STR_NO_RADIUS); 
                   printf(", ");
...
finish:
    ...
    {
        flint_printf("Usage:\n");
        flint_printf("%s x xnum nprimes thres sw y0 t0 d\n\n", argv[0]);
        flint_printf(
