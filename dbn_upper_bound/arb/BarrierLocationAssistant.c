...
int main(int argc, char *argv[])
{
    ...
    arb_t x, y, t, a, b, X, eulabs, thres, one, d, re, im, out, y0, t0, expmove0, expmovet0;
    ...
    for (tht=0; tht <= numx; tht++)
    {
        ...
        { 
            ...
            if (sw >= 1)
            {
                arb_set_si(expmove0, 1000);
                arb_set_si(expmovet0, 1000);
                for (i=0; i<3; i++)
                {
                   arb_zero(t);
                   arb_one(a);
                   ...
                   printf(", ");
                   arb_printn(a, 2, ARB_STR_NO_RADIUS);  
                   printf(", [");
                   arb_printn(t, 2, ARB_STR_NO_RADIUS);  
                   printf(", "); 
                   arb_printn(t0, 2, ARB_STR_NO_RADIUS);  
                   printf("], ["); 
                   if (sw>1)
                   {
                       N = (sw-1)*100000;       
                   ...
                   if (arb_lt(out, expmove0))
                       arb_set(expmove0, out); 
                   printf(", ");
                   abbeff_series(out1, b, a, t0, N, 1, prec);
                   acb_abs(out, out1, prec);
                   arb_printn(out, 10, ARB_STR_NO_RADIUS);
                   if (arb_lt(out, expmovet0))
                       arb_set(expmovet0, out); 
                   printf("]\n");
                }
                printf("Expected movement : ["); 
                arb_printn(expmove0, 10, ARB_STR_NO_RADIUS);
                printf(", "); 
                arb_printn(expmovet0, 10, ARB_STR_NO_RADIUS);
                printf("]\n");
...
finish:
    ...
    "outputs 6 abbeff values all for y=1, but for t=0 and t=t0 (and the mimimum difference).\n"
    "When the switch is set to >1 the abbeff values are calculated for (sw-1)*100K terms.\n");
    }
