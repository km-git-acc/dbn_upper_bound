...
arb_poly_abbeff_sawtooth_lbound(arb_poly_t lbound, const arb_poly_t t, const arb_poly_t y, 
                                arb_poly_srcptr at, arb_poly_srcptr bt, arb_poly_srcptr moll,
                                ...
                                slong D, slong nextN, arb_poly_t modgamend, arb_poly_t prevbound, slong n, slong prec)
{
    ...
    arb_poly_t ndsum_a, ndsummand_a, ndsummand, a, b;
    ...
    arb_init(lognd);
           ...
           for (k = 1; k <= d[v]; k++)
           {
              ...
              for (nd = 1; nd <= divs; nd++)
              {
                  if (((k+d[v]*(nextN-1)) % d[nd] == 0) && ((k+d[v]*(nextN-1)) <= d[nd]*nextN))
                  {
                      ...
                      arb_poly_mullow(ndsummand_b, moll + nd, bt + j, n, prec);
                      arb_poly_add_series(ndsum_b, ndsum_b, ndsummand_b, n, prec);
 
                      arb_poly_mullow(ndsummand_a, moll + nd, at + j, n, prec);
       ...
       arb_poly_mullow(sum2, bt + nextN, b, n, prec);
        ...
        slong D, slong Nmin, arb_poly_t modgamend, slong n, slong prec)
{
    ...
    for (k = 1; k <= Nmin; k++)
    {
        ...
        arb_poly_exp_series(kterm1, kterm1, n, prec);
        arb_poly_mullow(b, kterm1, kterm, n, prec);
        arb_poly_mullow(b, bt+k, b, n, prec);
          ...
          slong nprimes, arb_t threshold, slong mode, slong digits, slong n, slong prec)
{
    ...
    divs=0; D=0; bt=0; at=0;
 
    high_prec = digits * 3.32192809488736 + 10;

    Nmax = Nb;
 
    arb_t ar, a, logk, coeff;
    arb_init(ar);
    arb_init(a);
    arb_init(logk);
    arb_init(coeff);

    arb_poly_t tpoly, ypoly, xN, xN0, eA, eB, eC0, eTot, lboundfull, lboundinc, modgamend, Napoly, Nbpoly, nextNpoly, prevbound;
    arb_poly_init(tpoly);
    arb_poly_init(ypoly);
    arb_poly_init(xN0);
    arb_poly_init(xN);
    ...
    arb_poly_abbeff_base_lbound(lboundfull, tpoly, ypoly, at, bt, moll, nprimes, primes, d, D, Na, modgamend, 1, prec);
    ...
    while (nextN < Nb)
    {
        ...
        arb_poly_abbeff_sawtooth_lbound(lboundinc, tpoly, ypoly, at, bt, moll, nprimes, primes, d, D, nextN, modgamend, prevbound, 1, prec);
        ...
        {
            arb_poly_abbeff_base_lbound(lboundfull, tpoly, ypoly, at, bt, moll, nprimes, primes, d, D, nextN, modgamend, 1, prec);
            arb_poly_set(prevbound, lboundfull);
            flint_printf("%ld, ", nextN);
            arb_poly_get_coeff_arb(coeff, lboundfull, 0);
            arf_printd(arb_midref((coeff)), digits);
