...
\subsection{Proof of claim (i)}

The function $f_t(x+iy)$ is holomorphic, so by Rouche's theorem, it suffices to show that for each time $0 \leq t \leq 0.2$, as $x+iy$ traverses the boundary $\partial R$ of the rectangle 
...
The number of rectangle mesh points varies with $t$ ranging from $11076$ at $t=0$ to $56$ at $t=0.195$:
...
\subsection{Proof of claim (ii)}

For $N \in [69098,80000]$, the bound \eqref{x-adv} with the Euler $5$ mollifier (\eqref{ld} with $P=5$) gives a lower bound for $|f_t(x+iy)|$ by a quantity $F_N$ (the right-hand side of \eqref{x-adv}), which only depends on $N$.  A direct calculation then shows that $F_N > 0.042$; in fact $F_N$ monotonically increased with $N$ in this range, from $0.04278$ to $0.08011$.

\subsection{Proof of claim (iii)}

If $F_N$ is the quantity in claim (ii), one can easily compute $F_N$ for any given $N$ in the range $80000 \leq N \leq 1.5 \times 10^6$ to obtain a satisfactory lower bound.  However, the number of $N$ here is prohibitively large to permit a direct computation of each value of $F_N$.  To reduce the number of values of $N$, we establish upper bounds on the difference
$$ F_N - F_{N+1}$$
to prevent $F_N$ from decreasing too rapidly as $N$ advances.
...
$$ |f_{N+1}| \geq |f_{N}| - \sum \textrm{(additional terms corresponding to N+1)}$$
with the bound for $|f_{N_{\mathrm{min}}}|$ computed similarly to (77) or (78).


If the incremental bound goes below a positive threshold at $N$, we reset $N_{\mathrm{min}}$ to $N$ and restart the process, generating a sawtooth pattern.


For verifying the lower bounds for $[N_{\mathrm{min}},N_{\mathrm{max}}]$, and with
...
$g(n,N,N_{\mathrm{max}}) = \max(\frac{1-|\gamma|_{N_{\mathrm{max}}}}{1+|\gamma|_{N_{\mathrm{max}}}} |\beta_{n,N} + \alpha_{n,N}|, |\beta_{n,N} - \alpha_{n,N}|,  |\beta_{n,N} - |\frac{\gamma_{N_{\mathrm{max}}}}{\gamma_{N}}| \alpha_{n,N}|)$,

For the Triangle inequality bound,

$$|f_{N_{\mathrm{min}}}| \geq \frac{1}{\sum_{d=1}^D \frac{|\lambda_d|}{d^{\sigma_{N_{\mathrm{min}}}}}} (1 - |\gamma_{N_{\mathrm{min}}}| - \sum\limits_{n=2}^{DN_{\mathrm{min}}} \frac{|\beta_{n,N_{\mathrm{min}}}| + |\alpha_{n,N_{\mathrm{min}}}|}{n^{\sigma_{N_{\mathrm{min}}}}}) - |\gamma_{N_{\mathrm{min}}}| \sum_{n=1}^{N_{\mathrm{min}}} \frac{b_n^t (n^{|\kappa|} - 1)}{n^{\sigma_{N_{\mathrm{min}}}-y}}$$
...
$$|f_{N_{\mathrm{min}}}| \geq \frac{1}{\sum_{d=1}^D \frac{|\lambda_d|}{d^{\sigma_{N_{\mathrm{min}}}}}} (1 - |\gamma_{N_{\mathrm{min}}}| - \sum\limits_{n=2}^{DN_{\mathrm{min}}}  \frac{g(n,N_{\mathrm{min}},N_{\mathrm{max}})}{n^{\sigma_{N_{\mathrm{min}}}}} ) - |\gamma_{N_{\mathrm{min}}}| \sum_{n=1}^{N_{\mathrm{min}}} \frac{b_n^t (n^{|\kappa|} - 1)}{n^{\sigma_{N_{\mathrm{min}}}-y}}$$


$$|f_{N+1}| >= |f_{N}| - \frac{1}{\sum_{d=1}^D \frac{|\lambda_d|}{d^{\sigma_{N+1}}}} \sum\limits_{1 \leq d \leq D:d|D}(\sum\limits_{n=dN+1}^{dN+d} \frac{g(n,N_{\mathrm{min}},N_{\mathrm{max}})}{n^{\sigma_{N+1}}})  - |\gamma_{N+1}| \frac{b_{N+1}^t ({(N+1)}^{|\kappa|} - 1)}{(N+1)^{\sigma_{N+1}-y}}$$
...
2) $g(n,N,N_{\mathrm{max}})$ can be seen to have the following properties with n fixed and N varying, resulting in it being constant or decreasing with increasing N,

$\bullet g(n,N,N_{\mathrm{max}}) =  |\beta_{n,N} - \alpha_{n,N}| \textrm{ when } \beta_{n,N} \alpha_{n,N} \leq 0$

$\bullet g(n,N,N_{\mathrm{max}}) =  \max(\frac{1-|\gamma|_{N_{\mathrm{max}}}}{1+|\gamma|_{N_{\mathrm{max}}}} |\beta_{n,N} + \alpha_{n,N}|,  |\beta_{n,N} - |\frac{\gamma_{N_{\mathrm{max}}}}{\gamma_{N}}| \alpha_{n,N}|)) \textrm{ when } \beta_{n,N} \alpha_{n,N} > 0 \textrm{ and } |\beta_{n,N}| \geq |\alpha_{n,N}|$

$\bullet g(n,N,N_{\mathrm{max}}) =  \max(\frac{1-|\gamma|_{N_{\mathrm{max}}}}{1+|\gamma|_{N_{\mathrm{max}}}} |\beta_{n,N} + \alpha_{n,N}|, |\beta_{n,N} - \alpha_{n,N}|,  |\beta_{n,N} - |\frac{\gamma_{N_{\mathrm{max}}}}{\gamma_{N}}| \alpha_{n,N}|)) \textrm{ when } \beta_{n,N} \alpha_{n,N} > 0 \textrm{ and } |\alpha_{n,N}| > |\beta_{n,N}|$

$ $

Now, with $N_{\mathrm{max}} = 1.5 \times 10^6, y =0.2, t=0.2, \textrm{ Euler 5 mollifier, and threshold } = 0.03$

and starting with $N_{\mathrm{min}} = 80000$, and resetting $N_{\mathrm{min}}$ whenever the incremental lemma bound goes below 0.03, 

it was computationally verified that $|f_{N}| \geq 0.03$ for $N \in [80000,1.5 \times 10^6]$.  
...
\begin{align*}
\sigma &\geq 0.6 + \frac{0.2}{4} \log \frac{x_N}{4\pi} - \frac{0.2}{2x_{1.5 \times 10^6}^2} \left(0.4+\frac{0.96}{x_{1.5 \times 10^6}^2}\right)_+ \\
&\geq 0.6 + 0.1 \log N + 0.05 \log \left(1 - \frac{t}{16 (1.5 \times 10^6)^2}\right) - \frac{0.2}{2x_{1.5 \times 10^6}^2} \left(0.4+\frac{0.96}{x_{1.5\times 10^6}^2}\right)_+ \\
