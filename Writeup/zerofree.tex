...
To prove this theorem, we apply Theorem \ref{ubc} with $t_0 := 0.2$, $X := 5 \times 10^9$, and $y_0 = 0.4$.  It will thus suffices to verify the three conditions of the theorem.  For the first claim - namely, that $H_0(x+iy) \neq 0$ when $0 \leq x \leq 5 \times 10^9$ and $y \geq 0.4$ - this is immediate from this follows from \eqref{hoz}, \eqref{sas}, and the result of Platt that all the non-trivial zeroes of $\zeta$ with imaginary part between $0$ and $3.06 \times 10^{10}$ lie on the critical line $\{ \mathrm{Re}(s) = 1/2\}$.

For the remaining claims, one has to verify $H_t(x+iy) \neq 0$ for various $(x,y,t)$ in the region \eqref{region}.
To show that $H_t(x+iy) \neq 0$, it suffices by \eqref{ratio-form}, \eqref{xco} and the triangle inequality to show that
...
\end{equation}
and $s_*, \kappa, \gamma, e_A, e_B, e_{C_0}$ are given by \eqref{lambda-def}-\eqref{ec-def}.  
...
\begin{lemma}\label{lac}  If $x \geq 5 \times 10^5$, $0 \leq t \leq 0.2$, and $0.4 \leq y \leq 1$, then
$$ e_A + e_B + e_{C,0} \leq 4.37 \times 10^{-4} + 3.23 \times 10^{-10} (F_{N,0.2}(0.7 + \frac{1}{20} \log \frac{x}{4\pi}) + |\gamma| F_{N,0.2}( 0.3 + \frac{1}{20} \log \frac{x}{4\pi} ) ).$$
where
...
\begin{proof}
From Proposition \ref{estimates}(iv), (v) one has
...
From \eqref{delta1-bound} one has
\begin{align*}
...
Since $F_{N,t}(\sigma)$ is non-decreasing in $t$ and non-increasing in $\sigma$, the claim follows.
\end{proof}
...
We begin with claim (i).  We need some derivative estimates on the quantity $X = X( t, x+iy )$.

\begin{lemma} In the region \eqref{region}, we have
$$ |\frac{\partial X}{\partial x}| = |\frac{\partial X}{\partial y}| \leq  \sum_{n=1}^N \frac{b_n^t}{n^{\mathrm{Re}(s_*)}} (\frac{\log n}{2} + \frac{t \log n}{4(x-6)}) + |\gamma| N^{|\kappa|} \sum_{n=1}^N \frac{b_n^t n^{y} }{n^{\mathrm{Re}(s_{*})}}
...
then
$$ 
X = \sum_{n=1}^N \frac{b_n^t}{n^{s_*}} + \gamma \sum_{n=1}^N \frac{b_n^t}{n^{s_{**}}}.$$
...
$$ 
\frac{\partial X}{\partial x} = - \sum_{n=1}^N \frac{b_n^t}{n^{s_*}} \frac{\partial s_*}{\partial x} \log n + \gamma \sum_{n=1}^N \frac{b_n^t}{n^{s_{**}}}
...
$$ 
|\frac{\partial X}{\partial x}| \leq \sum_{n=1}^N \frac{b_n^t}{n^{\mathrm{Re}(s_*)}} (\frac{\log n}{2} + \frac{t \log n}{4(x-6)}) + |\gamma| \sum_{n=1}^N \frac{b_n^t}{n^{\mathrm{Re}(s_{**})}}
...
Writing $\mathrm{Re}(s_{**}) = \mathrm{Re}(s_*) - y + \mathrm{Re}(\kappa)$, we then have the claim.
\end{proof}
