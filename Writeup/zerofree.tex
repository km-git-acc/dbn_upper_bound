...
so in particular (recalling that $s_+ = \frac{1+y-ix}{2}$ and $s_- = \frac{1-y+ix}{2}$)
$$ \alpha(s_-) - \alpha(s_+) = \frac{\pi i}{2} + O_{\leq}( \frac{8}{x-6} )$$
and
$$ \alpha(s_-) + \alpha(s_+) = \log \frac{x}{4\pi} + O_{\leq}( \frac{8}{x-6} )$$
so that
$$ \left|\alpha(s_-)^2 - \alpha(s_+)^2\right| \leq \left(\frac{\pi}{2} + \frac{8}{x-6}\right) \left(\log \frac{x}{4\pi} + \frac{8}{x-6}\right).$$
...
{\bf describe the numerical output of this algorithm, for instance discussing the values of $t$ used, the size of the mesh at different points in $t$, bounds for $D_t, D_z$, etc.}
