...
Now we attend to the left edge of $\partial R$, in which $x = X_0 - 0.5$.
...
The product of $E_{t,7}(x+iy)$ and $\sum_{n=1}^N (n/N)^{0.2} \frac{b_n^t}{n^{\sigma-iT}}$ is not favorable due to the negative sign in the $\sigma-iT$ exponent.  But since $|z| |w| = |z \overline{w}|$, we have
\begin{align*}
...
%
%
%For $69098 \leq N \leq N_1$, the bound \eqref{x-adv} with the Euler $7$ mollifier (\eqref{ld} with $P=7$) gives a lower bound for $|f_t(x+iy)|$ by a quantity $F_N$ (the right-hand side of \eqref{x-adv}), which only depends on $N$.  One can easily compute $F_N$ for any given $N$ in the range $69098 \leq N \leq N_1$ to obtain a satisfactory lower bound.  We claim that $F_N \geq 0.03$ for all $N$ in this range, which suffices for our application.  A difficulty arises because the number of possible values of $N$ is too large for each $F_N$ to be evaluated numerically.  To reduce the number of values of $N$ at which such an evaluation occurs, we establish upper bounds on the difference
%$$ F_N - F_{N+1}$$
%to limit the extent to which $F_N$ decreases as $N$ advances.  These upper bounds are faster to compute than $F_N$ itself, and allow one to deduce lower bounds on $F_{N+1}$ quickly from lower bounds from $F_N$.  We iterate this procedure until the lower bound drops below the target threshold $0.03$, at which point we calculate $F_N$ again and restart the process.   This ``sawtooth'' procedure turns out to efficiently establish the bound $F_N \geq 0.03$ for all $N$ in this range.
%
%
%
