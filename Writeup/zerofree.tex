...
$$ e_A + e_B + e_{C,0} \leq 1.249 \times 10^{-3} + 2.124 \times 10^{-11} F_{N,0.2}( 0.7001 + \frac{t}{4} \log \frac{x}{4\pi} )
$$
where
...
$$ e_A + e_B + e_{C,0} \leq 1.251 \times 10^{-3}.$$
\end{lemma}
...
\leq 1.2 \times 10^{-23}$$
to obtain t
$$ \mathrm{Re}(s_*) \geq 0.7001 + \frac{t}{4} \log \frac{x}{4\pi}$$
(say).  Since $F_{N,t}(\sigma)$ is non-increasing in $\sigma$, we conclude
$$  e_A + e_B \leq 2.124 \times 10^{-11} F_{N,0.2}( 0.7001 + \frac{t}{4} \log \frac{x}{4\pi} ).$$
...
\end{split}
\end{equation}
where we have discarded the negative term $- \frac{t}{16} \log^2 \frac{X_0-0.5}{4\pi}$.  
The first claim follows.  For the second claim, we have in the region (ii) that $N=N_0$, and we may crudely bound
$$
F_{N,0.2}( 0.7001 + \frac{t}{4} \log \frac{x}{4\pi} ) \leq F_{N_0,0.2}(0.7001) \leq 10538$$
and then the second claim follows from the first.
\end{proof}
