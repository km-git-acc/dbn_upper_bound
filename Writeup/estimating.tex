...
$$ \exp( a ) = \sum_{j=0}^T \frac{a^j}{j!} + O_{\leq}\left( \frac{|a|^{T+1}}{(T+1)!} \exp(|a|) \right)$$
and hence
$$F_N(z_0+\zeta, w) = F_{N_0}(z_0+\zeta, w) + \sum_i \sum_{-H/2 \leq h \leq H/2} 
...
The point is that it only requires $O\left( \frac{N}{H} T H \right)$ calculations to compute the $\beta_{i,j,h}$, and $O\left( \frac{N}{H} T M \right)$ calculations to compute the $\sigma_{i,j}(\zeta,w)$, so the total computation time is now
$$O\left( N_0 M + \frac{N}{H} T H + \frac{N}{H} T M + \frac{N}{H} T M \right) = O\left( NM \left( \frac{N_0}{N} + \frac{T}{M} + \frac{T}{H} \right) \right)$$
...
$$|\varepsilon_{i,h}| \leq |\varepsilon_{i,-H/2}| = \log \frac{N_i}{N_i-H/2} \leq \log\left(1 + \frac{H}{2N_0}\right)$$
and
...
$$
where
$$ \delta := (|\zeta| + 2 |w| \log N) \log\left(1+\frac{H}{2N_0}\right) + |w| \log^2\left(1+\frac{H}{2N_0}\right).$$
