...
\end{equation}
for any fixed $t>0$ and $y>0$; see Figure \ref{ht-bt}.  

\begin{figure}[h!]
  \includegraphics[width=1.0\linewidth]{Ht_vs_Bt_variousregimes.png}
  \caption{The quantity $|H_t(x+iy)|/|B_t(x+iy)|$ when $y=t=0.4$ and $\log_{10} x \leq 12$ (left) and when for $1000 \leq x \leq 1030$ and various choices of $(y,t)$ (right).}
\label{ht-bt}
\end{figure}

\begin{figure}[h!]
  \includegraphics[width=0.8\linewidth]{LogHt_Log_Bt.png}
  \caption{The quantities $\log|H_t(x+iy)|$ and $\log|B_t(x+iy)|$ when $y=t=0.4$ and $\log_{10} x \leq 7$. This illustrates how $B_t$ captures the bulk of the exponential decay of $H_t$.}
\label{loghtandlogbt}
\end{figure}
...
{\bf I think it would also be good to have a plot of $e_A, e_B, e_{C,0}, e_A+e_B+e_{C,0}$, and $|f_t - H_t/B_t|$ for a fairly large range of $x$ (maybe using log-scale in $x$).}
...
$$ N_t(X) = \left\lfloor  \frac{X}{4\pi} \log \frac{X}{4\pi} - \frac{X}{4\pi} + \frac{11}{8} + \frac{t}{16} \log \frac{X}{4\pi} + O( X^{-ct} ) \right\rfloor$$
...
{\bf perhaps have a graphic comparing the actual zeroes of $H_t$ with the places where $\frac{X}{4\pi} \log \frac{X}{4\pi} - \frac{X}{4\pi} + \frac{11}{8} + \frac{t}{16} \log \frac{X}{4\pi}$ is an integer?}

\begin{figure}[h!]
  \includegraphics[width=1.0\linewidth]{Integerconvergence_t_and_x_direction.png}
  \caption{The real zeroes $x_i$ of $H_t$ will convergence to integer values of $g(x_i,t)= \frac{x_i}{4\pi} \log \frac{x_i}{4\pi} - \frac{x_i}{4\pi} + \frac{11}{8} + \frac{t}{16} \log \frac{x_i}{4\pi}$ when $t$ (left) and/or $x$ (right) increases.}
