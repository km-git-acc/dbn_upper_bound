\section{Initial approximations}\label{initial-sec}

In this section we give some initial estimates on the functions $r_{t,n}, R_{t,N}$ appearing in Section \ref{heatflow-sec}, and use this to approximate $H_t$.  
...
\begin{corollary}[$A+B-C$ approximation]\label{abc-cor}  Let $0 \leq y \leq 1$, $x \geq 200$, and $0 < t \leq 1/2$.  Set
...
In our application we will just need the cruder ``$A+B$'' approximation that is immediate from the above corollary and \eqref{cop}:
...
\frac{H_t(x+iy)}{B_0(x+iy)} = \sum_{n=1}^N \frac{b_n^t}{n^{s_*}} + \lambda \sum_{n=1}^N n^y \frac{b_n^t}{n^{\overline{s_*} + \kappa}} + O_{\leq}\left( e_A + e_B + e_{C,0} \right)
\end{equation}
where
\begin{align}
\lambda \coloneqq \lambda(x+iy) &\coloneqq \frac{M_t(\frac{1-y+ix}{2})}{M_t(\frac{1+y-ix}{2})} \label{lambda-def} \\
...
e_A \coloneqq e_A(x+iy) &\coloneqq |\lambda| \sum_{n=1}^N n^y \frac{b_n^t}{n^{\mathrm{Re}(s) + \mathrm{Re}(\kappa)}} \eps_{t,n}(\frac{1-y+ix}{2}) \label{ea-def}\\
...
\begin{proposition}  Let the notation and hypotheses be as in Corollary \ref{abc-cor}, and let $\lambda, \kappa, e_A, e_B, e_{C,0}$ be defined by \eqref{lambda-def}-\eqref{ec-def}.
\begin{itemize}
\item[(i)]  One has
$$ |\lambda| \leq e^{0.02 y} \left( \frac{x}{4\pi} \right)^{-y/2} $$
\item[(ii)] One has
$$ \mathrm{Re} s_* \geq \frac{1+y}{2} +\frac{t}{2} \log \frac{x}{4\pi} - \frac{(1-3y)_+ t}{2x^2}.$$
\item[(iii)]  One has
$$ \kappa = O_{\leq} \left( \frac{ty}{2(x-6)} \right).$$
\item[(iv)]  One has
$$ e_A \leq e^{0.02 y} \left( \frac{x}{4\pi} \right)^{-y/2} \sum_{n=1}^N n^{y + \frac{ty}{2(x-6)}} \frac{b_n^t}{n^{\mathrm{Re}(s)}} \left( \exp\left( \frac{\frac{t^2}{32} \log^2 \frac{x}{4\pi n^2} + 0.313}{T-3.33} \right)-1 \right).$$
\item[(v)]  One has
$$ e_B \leq \sum_{n=1}^N  \frac{b_n^t}{n^{\mathrm{Re}(s)}} \left( \exp\left( \frac{\frac{t^2}{32} \log^2 \frac{x}{4\pi n^2} + 0.313}{T-3.33} \right)-1 \right).$$
\item[(vi)] One has
$$ e_C \leq \left(\frac{T'}{2\pi}\right)^{-\frac{1+y}{4}} \exp\left( - \frac{t}{16} \log^2 \frac{x}{4\pi} \right)
(1.46 + \frac{1.33 \times (3^y+3^{-y})}{a-0.125}).
$$
\end{itemize}
\end{proposition}
...
$$ \log \lambda = -y \frac{d}{d\sigma} \log \left|M_t\left( \sigma + \frac{ix}{2}\right)\right| $$
...
$$ \log \lambda = -y \mathrm{Re} \left( \left(\frac{1}{2} \log \frac{x}{4\pi} + i \frac{\pi}{4} + O_{\leq}\left( \frac{2+\sigma}{x-6} \right)\right) \left(1 + O_{\leq}\left(\frac{t}{2(x-6)}\right)\right) \right).$$
Expanding this out, we have
$$ \log \lambda = -y (\frac{1}{2} \log \frac{x}{4\pi} + O_{\leq}\left( \frac{2+\sigma + \frac{t}{4} \log \frac{x}{4\pi} + \frac{t\pi}{8} + \frac{t(2+\sigma)}{2(x-6)}}{x-6} \right)).$$
With $\sigma \leq 1$, $0 < t \leq 1/2$, and $x \geq 200$ one has
$$ 2 + \sigma + \frac{t\pi}{8} + \frac{t(2+\sigma)}{2(x-6)} \leq 3.21$$
and thus
$$ \log \lambda \leq -\frac{y}{2} \log \frac{x}{4\pi} + y \frac{\frac{t}{4} \log \frac{x}{4\pi} + 3.21}{x-6}.$$
...
since $x \geq 200$ and $0 < t \leq 1/2$, the right-hand side is certainly bounded by $(\frac{x}{4\pi})^2$, so that
$$ \frac{4\pi}{x} \leq \frac{x}{4\pi n^2} \leq \frac{x}{4\pi}$$
and hence
$$ |\log \frac{x}{4\pi n^2} + i \frac{\pi}{2}| \leq |\log \frac{x}{4\pi} + i \frac{\pi}{2}|$$
and for $x \geq 200$ one can check that $\frac{|\log \frac{x}{4\pi} + i \frac{\pi}{2}|}{x-6}$ is decreasing in $x$.  Thus
\begin{align*}
...
&\leq 0.667.
\end{align*}
For $0 < t \leq \frac{1}{2}$ we also have
...
As before we have
\begin{align*}
\frac{3 |\log \frac{x}{4\pi n^2} + i \frac{\pi}{2}|}{x-6} + \frac{9}{(x-6)^2} &\leq \frac{3 |\log \frac{200}{4\pi n^2} + i \frac{\pi}{2}|}{200-6} + \frac{9}{(200-6)^2} \\
&\leq 0.05
\end{align*}
and thus
\begin{align*}
\frac{\exp\left( \frac{t \pi^2}{64}\right) |M_0(iT')|}{|M_t(\frac{1+y+ix}{2})|}
&= \left(\frac{T'}{2\pi}\right)^{-\frac{1+y}{4}} \exp\left( - \frac{t}{16} \log^2 \frac{x}{4\pi} + O_{\leq}\left( 0.05 + \frac{3.58}{x-6} \right) \right)\\
&\leq \left(\frac{T'}{2\pi}\right)^{-\frac{1+y}{4}} \exp\left( - \frac{t}{16} \log^2 \frac{x}{4\pi} + 0.069 \right).
\end{align*}
Finally, since $T' \geq \frac{x}{2} \geq 100$, one has 
$$\exp\left(\frac{3.49}{T'-3.33}\right) \leq 1.037$$
and
$$ \frac{5}{3(T'-3.33)} \leq 0.173$$
and hence by \eqref{epsp-def}
$$ 
\tilde \eps(\frac{1 \pm y+ix}{2}) \leq \frac{1.24 \times 3^{\pm y}}{a-0.125} + 0.18$$
and hence
$$
\exp(0.069) (1 + \tilde \eps(\frac{1-y+ix}{2}) + \tilde \eps(\frac{1+y+ix}{2})) \leq 1.46 + \frac{1.33 \times (3^y+3^{-y})}{a-0.125}$$
giving the claim.
