...
\end{equation}
The bounds in this region are somewhat arbitrary; one could easily modify the arguments below to extend to wider regions, at the expense of making the numerical bounds in the effective estimates worse (or more complicated).
...
where
\begin{align}
\gamma \coloneqq \lambda(x+iy) &\coloneqq \frac{M_t(\frac{1-y+ix}{2})}{M_t(\frac{1+y-ix}{2})} \label{lambda-def} \\
...
\begin{proposition}[Estimates]\label{estimates}  Let the notation and hypotheses be as in Corollary \ref{abc-cor}, and let $\gamma, \kappa, e_A, e_B, e_{C,0}$ be defined by \eqref{lambda-def}-\eqref{ec-def}.
\begin{itemize}
\item[(i)]  One has
...
\end{align*}
Finally, since $T' \geq \frac{x}{2} \geq 100$ in \eqref{region}, one has 
